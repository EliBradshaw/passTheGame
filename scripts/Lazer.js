...
export default class Lazer {
    ...
    centeredPlayerPosition() {
        return this.player.position.copy()
            .add(new Vector(0, 0)) // Lazer centering, if needed
    }
    ...
    updateLazer() {
        ...
        this.lazerEnd = this.centeredPlayerPosition()
            .copy()
            .add(direction);

        for (let enemy of this.game.enemies) {
            let emergency = 0;
            while (Collision.lineIntersectsCircle(this.centeredPlayerPosition(), this.lazerEnd, enemy.position, enemy.radius)) {
                enemy.position.add(direction.scale(0.1));
                if (emergency++ > 100) break;
            }
            if (emergency && this.game.isSwapped) // Also useful to check if it ever collided
      ...
      renderLazer(ctx) {
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(this.centeredPlayerPosition().x, this.centeredPlayerPosition().y);
